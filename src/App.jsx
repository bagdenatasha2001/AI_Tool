...
import RecentSearch from './Compoents/RecentSearch';
import QuestionsAnswer from './Compoents/QuestionsAnswer';
...
export default function App() {
  ...
  const Askquestion = async () => {
    ...
    try {
      ...
      let dataString = response?.candidates?.[0]?.content?.parts?.[0]?.text || "⚠️ No valid response received.";
  ...
  return (
    <>
      <div className={darkMode === 'dark' ? 'dark' : 'light'}>
        <div className='grid grid-cols-5 h-screen text-center overflow-scroll  '>

          <select
            onClick={(event) => setDarkMode(event.target.value)}
            className='bg-clip-text text-transparent bg-gradient-to-r from-pink-700 to-violet-700 fixed bottom-1 p-5 outline-none '>
          ...
          <div className='col-span-4 p-10'>
            <h1 className='text-4xl p-1 bg-clip-text text-transparent bg-gradient-to-r from-pink-700 to-violet-700'>
            ...
            <div ref={scrollToAns} className='container h-110 overflow-scroll no-scrollbar'>
            ...
            <div className='flex dark:bg-zinc-800 w-1/2 bg-red-100 dark:text-white text-zinc-800 m-auto rounded-4xl border border-zinc-700 p-1 pr-5 h-12'>
              <input
                type="text"
